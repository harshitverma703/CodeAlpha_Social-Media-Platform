...
async function loadSidebarUser() {
  try {
    ...
    document.getElementById("sidebarBio").textContent =
      user.bio || "Saturn user";
  } catch (error) {
...
function setupCreatePost() {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    if (!contentInput) {
      showToast("Post content field not found.");
      return;
    }
  ...
  return `
    <article class="card post">
      ...
      <div class="post-header">
        ...
        <a
          class="user-meta"
          href="profile.html?username=${encodeURIComponent(post.username)}"
        >
          <img
            class="avatar"
            src="${avatarPath(post.avatar)}"
            alt="Avatar"
          >
      ...
      <div class="post-actions">
        ...
        >
          <i
            class="fa-${post.liked_by_me ? "solid" : "regular"} fa-heart"
          ></i>
...
async function handleLike(event) {
  ...
  try {
    const data = await apiFetch(`/likes/${postId}`, {
      method: "POST"
    });
...
async function handleDeletePost(event) {
  const postId =
    event.currentTarget.dataset.deletePost;
